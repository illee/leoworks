...
> 한 줄: 회사 사이트(leoworks.kr) **HTTPS 라이브 + 메일 포워딩 완료** 상태 유지. 오늘 = **홈페이지에 캐주얼 게임 블럭(낱말채집·하루 스도쿠) 추가**(커밋 완료, **배포는 대기**).
...
| 영역 | 상태 |
|---|---|
| 회사 사이트 `leoworks.kr` | ✅ HTTPS 라이브. **오늘 캐주얼 게임 블럭 추가(로컬 커밋 `62ea047`, 미배포)** |
...
- 데스크톱·모바일 로컬 렌더링 미리보기 확인. **커밋 `62ea047`(로컬), origin 미푸시.**

## 2. ⏳ 미해결 / 다음 액션 (배포 게이트)
- **배포 결정 대기**: ① 배포(push) 진행할지 ② **배지 문구** — 게임은 Android 우선이라 "Google Play 출시 예정"으로 했으나, 다른 제품처럼 "Google Play · App Store"로 통일할지. → 사용자 확정 후 `git push origin main`(Pages 자동 재빌드).
- 스도쿠 실제 런처 아이콘 생성(별도, sudoku-mb-2606).
